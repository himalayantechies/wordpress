...
class TaskHelper extends MvcHelper{
    ...
    public function admin_actions_cell($controller, $object, $options = array()) {
        ...
        if($options['actions']['add_media']){
            $links[] = '<a href="'.MvcRouter::admin_url(array('object' => $object, 'action' => 'add_media')).'" title="' . __('Add Media', 'wpmvc') . ' ' .$encoded_object_name.'" target="_blank">' . __('Media', 'wpmvc') .'</a>';
        }
        ...
        if($options['actions']['view']){
            $links[] = '<a href="'.MvcRouter::admin_url(array('object' => $object, 'action' => 'view')).'" title="' . __('View', 'wpmvc') . ' ' .$encoded_object_name.'">' . __('View', 'wpmvc') .'</a>';
        }
